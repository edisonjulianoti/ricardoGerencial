...
class EmailTemplateHeaderList extends TPage
{
    private $form; // form
    ...
    private static $activeRecord = 'EmailTemplate';
    private static $primaryKey = 'id';
    private static $formName = 'formList_EmailTemplate';
    ...
    public function __construct($param = null)
    {
        ...
        $action_onDelete = new TDataGridAction(array('EmailTemplateHeaderList', 'onDelete'));
        ...
        $panel = new TPanelGroup("Listagem de email templates");
        ...
        $dropdown_button_exportar->addPostAction( "CSV", new TAction(['EmailTemplateHeaderList', 'onExportCsv'],['static' => 1]), self::$formName, 'fas:file-csv #00b894' );
        $dropdown_button_exportar->addPostAction( "XLS", new TAction(['EmailTemplateHeaderList', 'onExportXls'],['static' => 1]), self::$formName, 'fas:file-excel #4CAF50' );
        $dropdown_button_exportar->addPostAction( "PDF", new TAction(['EmailTemplateHeaderList', 'onExportPdf'],['static' => 1]), self::$formName, 'far:file-pdf #e74c3c' );
        $dropdown_button_exportar->addPostAction( "XML", new TAction(['EmailTemplateHeaderList', 'onExportXml'],['static' => 1]), self::$formName, 'far:file-code #95a5a6' );
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Configurações","Email templates"]));
        }
    ...
    { 
        if(isset($param['delete']) && $param['delete'] == 1)
        {
            try
            {
                ...
                // instantiates object
                $object = new EmailTemplate($key, FALSE); 
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            // creates a repository for EmailTemplate
